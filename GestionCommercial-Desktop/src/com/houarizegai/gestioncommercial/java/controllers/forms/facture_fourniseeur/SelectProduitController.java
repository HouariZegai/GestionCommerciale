...
public class SelectProduitController implements Initializable {
    ...
    private void filterSearchTable() {
        tableProduit.setPredicate((TreeItem<TableProduit> fournisseur) -> {
            ...
            String prixHT = (fournisseur.getValue().prixHt.getValue() == null) ? "" : fournisseur.getValue().prixHt.getValue().toLowerCase();
            String tauxTva = (fournisseur.getValue().tauxTva.getValue() == null) ? "" : fournisseur.getValue().tauxTva.getValue().toLowerCase();
    ...
    private void onSelect() {
        ...
        for (Produit produit : produits) {
            if (produit.getReference().equalsIgnoreCase(refProduitSelected)) {
                FactureFournisseurController.selectedProduit = produit;
                break;
            }
        }

        FactureFournisseurController.dialogSelectProduit.close();
    }
